...
func Apply(w http.ResponseWriter, r *http.Request) {
	log := settings.Log.WithValues("action", "apply-rabbitmq")

	err := pkg.PrepareOOKValues(r, []string{"rabbitmq.yaml"})
	if err != nil {
	...
	cmd := exec.Command(settings.ActionsDir + "rabbitmq/apply")
...
func Status(w http.ResponseWriter, r *http.Request) {
	...
	status, err := pkg.StatusByPodLabel("release_group=rabbitmq")
	if err != nil {
		log.Error(err, "")
		http.Error(w, http.StatusText(http.StatusInternalServerError),
			http.StatusInternalServerError)
