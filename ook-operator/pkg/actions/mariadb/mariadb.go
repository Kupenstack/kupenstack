...
func Apply(w http.ResponseWriter, r *http.Request) {
	log := settings.Log.WithValues("action", "apply-mariadb")

	err := pkg.PrepareOOKValues(r, []string{"mariadb.yaml"})
	if err != nil {
	...
	cmd := exec.Command(settings.ActionsDir + "mariadb/apply")
...
func Status(w http.ResponseWriter, r *http.Request) {
	...
	status, err := pkg.StatusByPodLabel("release_group=mariadb")
	if err != nil {
		log.Error(err, "")
		http.Error(w, http.StatusText(http.StatusInternalServerError),
			http.StatusInternalServerError)
