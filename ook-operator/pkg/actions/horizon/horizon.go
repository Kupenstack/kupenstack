...
func Apply(w http.ResponseWriter, r *http.Request) {
	log := settings.Log.WithValues("action", "apply-horizon")

	err := pkg.PrepareOOKValues(r, []string{"horizon.yaml"})
	if err != nil {
	...
	cmd := exec.Command(settings.ActionsDir + "horizon/apply")
...
func Status(w http.ResponseWriter, r *http.Request) {
	...
	status, err := pkg.StatusByPodLabel("release_group=horizon")
	if err != nil {
		log.Error(err, "")
		http.Error(w, http.StatusText(http.StatusInternalServerError),
			http.StatusInternalServerError)
