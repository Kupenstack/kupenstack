...
func Apply(w http.ResponseWriter, r *http.Request) {
	log := settings.Log.WithValues("action", "apply-placement")

	err := pkg.PrepareOOKValues(r, []string{"placement.yaml"})
	if err != nil {
	...
	cmd := exec.Command(settings.ActionsDir + "placement/apply")
...
func Status(w http.ResponseWriter, r *http.Request) {
	...
	status, err := pkg.StatusByPodLabel("release_group=placement")
	if err != nil {
		log.Error(err, "")
		http.Error(w, http.StatusText(http.StatusInternalServerError),
			http.StatusInternalServerError)
