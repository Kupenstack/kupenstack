...
func Apply(w http.ResponseWriter, r *http.Request) {
	log := settings.Log.WithValues("action", "apply-neutron")

	err := pkg.PrepareOOKValues(r, []string{"neutron.yaml"})
	if err != nil {
	...
	cmd := exec.Command(settings.ActionsDir + "neutron/apply")
...
func Status(w http.ResponseWriter, r *http.Request) {
	...
	status, err := pkg.StatusByPodLabel("release_group=neutron")
	if err != nil {
		log.Error(err, "")
		http.Error(w, http.StatusText(http.StatusInternalServerError),
			http.StatusInternalServerError)
